...
public class BarCodeReadEvent extends Event<BarCodeReadEvent> {
  ...
  public static com.rnscanbarcode.RNCamera.camera.events.BarCodeReadEvent obtain(int viewTag, Result barCode, int width, int height, byte[] compressedImage) {
    com.rnscanbarcode.RNCamera.camera.events.BarCodeReadEvent event = EVENTS_POOL.acquire();
    if (event == null) {
      event = new com.rnscanbarcode.RNCamera.camera.events.BarCodeReadEvent();
    }
