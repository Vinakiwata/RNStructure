...
public class CameraReadyEvent extends Event<CameraReadyEvent> {
  ...
  public static com.rnscanbarcode.RNCamera.camera.events.CameraReadyEvent obtain(int viewTag) {
    com.rnscanbarcode.RNCamera.camera.events.CameraReadyEvent event = EVENTS_POOL.acquire();
    if (event == null) {
      event = new com.rnscanbarcode.RNCamera.camera.events.CameraReadyEvent();
    }
    event.init(viewTag);
    return event;
  }
